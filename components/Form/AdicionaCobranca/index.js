...
function CobrancaForm({ onClose }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const { data: mensagem } = await api.post('/cadastrocobranca', form, config);

      successMessage(mensagem);

      register.valor = `${formatToFormCurrency(register.valor)}`;

      setCobrancas([...cobrancas, register]);
      return onClose();
    } catch (error) {
      errorMessage(error.response.data);
      return console.log('erro>', error.response.data);
    }
  };
    ...
    <>
      <Form onSubmit={submit}>
        <Styled.Titulo>
          ...
          <Styled.Text style={{ marginLeft: '16px' }}>Cadastro de Cobrança</Styled.Text>
        </Styled.Titulo>
